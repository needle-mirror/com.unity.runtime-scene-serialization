...
namespace Unity.RuntimeSceneSerialization.CodeGen
{
    ...
    class Context
    {
        ...
        readonly Dictionary<Type, TypeReference> m_TypeToTypeReference = new();
        readonly Dictionary<TypeReference, TypeReference> m_TypeReferenceToTypeReference = new();
        ...
        public void AddInitializeOnLoadMethodAttribute(ICustomAttributeProvider provider)
        {
            if (IsEditor)
            {
                var initializeOnLoadMethodAttributeConstructor = typeof(InitializeOnLoadMethodAttribute).GetConstructor(Type.EmptyTypes);
            ...
            var runtimeInitializeOnLoadMethodAttributeConstructor = typeof(RuntimeInitializeOnLoadMethodAttribute).GetConstructor(new[]{typeof(RuntimeInitializeLoadType)});
            var runtimeInitializeOnLoadMethodAttribute = new CustomAttribute(Module.ImportReference(runtimeInitializeOnLoadMethodAttributeConstructor))
            {
                ...
                {
                    new CustomAttributeArgument(Module.ImportReference(typeof(RuntimeInitializeLoadType)), RuntimeInitializeLoadType.AfterAssembliesLoaded)
                }
            };
        ...
        {
            var preserveAttributeConstructor = typeof(PreserveAttribute).GetConstructor(Type.EmptyTypes);
