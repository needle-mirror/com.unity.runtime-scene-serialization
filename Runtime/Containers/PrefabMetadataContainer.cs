...
{
    [Serializable, GenerateRuntimeSceneSerializationPropertyBag]
    class PrefabMetadataContainer
    {
        ...
        public PrefabMetadataContainer(PrefabMetadata prefabMetadata)
        {
            ...
            PostProcessOverrideList(PropertyOverrides);
        }

        static void PostProcessOverrideList(List<RuntimePrefabPropertyOverride> list)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                switch (propertyOverride)
                {
                    case IRuntimePrefabOverrideUnityObject objectProperty:
                        objectProperty.ConvertToUnityObjectReferenceOverride(list, i);
                        break;
...
#if UNITY_EDITOR
        public PrefabMetadataContainer(GameObject gameObject)
        {
            PropertyOverrides = new List<RuntimePrefabPropertyOverride>();
            AssetPack.GetPrefabMetadata(gameObject, out var guid, AssetPack.CurrentAssetPack);
            m_Guid = guid;
            ...
            foreach (var over in overrides)
            {
                ...
                RuntimePrefabPropertyOverride.GetOverrides(instanceObject, PropertyOverrides, transformPath, componentIndex);
            }
            ...
            GetMetadataRecursively(gameObject);
        }

        void GetMetadataRecursively(GameObject gameObject, string parentTransformPath = "", string transformPath = "")
        {
            ...
            if (PrefabUtility.IsAddedGameObjectOverride(gameObject))
            {
                AddedGameObjects.Add(new RuntimeAddedGameObject
                {
                    ...
                    GameObject = new GameObjectContainer(gameObject)
                });
            }
            ...
            foreach (Transform child in transform)
            {
                ...
                GetMetadataRecursively(child.gameObject, transformPath, childTransformPath);
            }
        }
#endif
    }
