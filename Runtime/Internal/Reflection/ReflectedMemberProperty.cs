...
namespace Unity.RuntimeSceneSerialization.Internal
{
    ...
    readonly struct PropertyMember : IMemberInfo
    {
        ...
        internal PropertyInfo PropertyInfo => m_PropertyInfo;
    ...
    class ReflectedMemberProperty<TContainer, TValue> : Property<TContainer, TValue>
    {
        ...
        public ReflectedMemberProperty(FieldInfo info, string name) : this(new FieldMember(info, name), name)
        {
        ...
        /// <param name="name">Use this name property--this might override the MemberInfo name</param>
        public ReflectedMemberProperty(PropertyInfo info, string name) : this(new PropertyMember(info, name), name)
        {
        ...
        internal ReflectedMemberProperty(IMemberInfo info, string name)
        {
...
#if !ENABLE_IL2CPP && !UNITY_DOTSRUNTIME
            ...
            else if (m_Info is PropertyMember propertyMember)
            {
                if (m_IsStructContainerType)
                {
                    var getMethod = propertyMember.PropertyInfo.GetGetMethod(true);
                    m_GetStructValueAction = (GetStructValueAction)Delegate.CreateDelegate(typeof(GetStructValueAction), getMethod);
                    if (!isReadOnly)
                    {
                        var setMethod = propertyMember.PropertyInfo.GetSetMethod(true);
