...
namespace Unity.RuntimeSceneSerialization
{
    class GameObjectVisitor : JsonSceneReader
    {
        ...
        public GameObjectVisitor(UnsafeValueView view, List<DeserializationEvent> events, Transform parent, GameObjectContainer prefabRoot, JsonSerializationParameters parameters)
        {
            SetView(view);
            SetEvents(events);
            m_Events = events;
                ...
                using (CreateViewScope(value))
                {
                    if (container is GameObjectContainer gameObjectContainer)
                    {
                        ...
                        if (name == GameObjectContainer.ComponentsProperty)
                        {
                            ...
                            ((IPropertyAccept<TContainer>)property).Accept(new ComponentVisitor(gameObject, m_View, m_Events), ref container);

                            var metadata = gameObjectContainer.PrefabMetadataContainer;
                            if (metadata != null)
                            {
                                gameObjectContainer.FinalizePrefab(metadata);
                                m_PrefabRoot = null;
                            }
                        ...
                        {
                            ((IPropertyAccept<TContainer>)property).Accept(new GameObjectVisitor(m_View, m_Events, gameObjectContainer.GameObject.transform, m_PrefabRoot, m_Parameters), ref container);
                            return;
                        }
                    }
                    else if (container is RuntimeAddedComponent addedComponent)
                    {
                        if (name == RuntimeAddedComponent.ComponentFieldName)
                        {
                            ...
                            ((IPropertyAccept<TContainer>)property).Accept(new ComponentVisitor(target.gameObject, m_View, m_Events), ref container);
                            return;
                        }
                    }
                    else if (container is RuntimeAddedGameObject addedGameObject)
                    {
                        if (name == nameof(addedGameObject.GameObject))
                        {
                            ...
                            ((IPropertyAccept<TContainer>)property).Accept(new GameObjectVisitor(m_View, m_Events, parent, m_PrefabRoot, m_Parameters), ref container);
                            return;
                        }
                    }
                    else if (container is PrefabMetadataContainer prefabMetadataContainer)
                    {
                        if (name == PrefabMetadataContainer.GuidFieldName)
                        {
                            ...
                            ((IPropertyAccept<TContainer>)property).Accept(this, ref container);
                            m_PrefabRoot.InstantiatePrefab(prefabMetadataContainer.Guid);
                            return;
                        }
                    }
        ...
        {
            var visitor = new GameObjectVisitor(default, default, default, default, default);
