...
namespace Unity.RuntimeSceneSerialization
{
    class SceneVisitor : JsonSceneReader
    {
        ...
        public SceneVisitor(Transform sceneRoot, UnsafeValueView view, List<DeserializationEvent> events, JsonSerializationParameters parameters)
        {
            SetView(view);
            SetEvents(events);
            m_Events = events;
                ...
                using (CreateViewScope(value))
                {
                    if (property is Property<TContainer, List<GameObjectContainer>>)
                    {
                        ((IPropertyAccept<TContainer>) property).Accept(
                            new GameObjectVisitor(m_View, m_Events, m_SceneRoot, null, m_Parameters), ref container);
                    }
                    else
                    {
